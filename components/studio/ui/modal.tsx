...
const ModalCloseButton = ({
  ...
  <DialogPrimitive.Close
    className={cn(
      'bg-background border-border focus:ring-ring rounded-full border',
      'transition-[border-color,box-shadow] duration-150 active:scale-95',
      'p-2 transition-all duration-150 hover:border-rose-500/25',
      'hover:bg-rose-500/10 hover:text-rose-500 focus:ring-2',
      'active:ring-0',
...
const modalVariants = cva(
  ...
  cn(
    'fixed bg-background/15 dark:bg-background/90 backdrop-blur-sm z-[51] isolate data-[state=open]:animate-in',
    'data-[state=closed]:animate-out data-[state=closed]:ease-in transition-shadow duration-300 data-[state=closed]:shadow-sm',
    'data-[state=open]:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)] data-[state=open]:drop-shadow-[0_20px_25px_rgba(0,0,0,0.35)]',
  ),
  {
    variants: {
      variant: {
        default: [
          'left-[50%] top-[50%] max-w-lg translate-x-[-50%] translate-y-[-50%] grid overflow-hidden',
        ...
        'from-trigger': ['max-w-lg grid sm:rounded-3xl overflow-hidden'],
        drawer: [
          'bottom-0 rounded-t-3xl overflow-hidden',
          'data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom',
          'data-[state=closed]:duration-300 data-[state=open]:duration-400',
          'data-[state=open]:ease-[cubic-bezier(0.16,1,0.3,1)]',
        ],
        'slide-out': [
          'inset-y-0 right-0 overflow-hidden',
    ...
    return (
      <ModalPortal>
        ...
        >
          ...
          {showClose && (
            <div className="absolute top-[16px] right-[16px] flex items-center gap-3">
              <ModalCloseButton
                className={cn(
                  'border-border focus:ring-ring rounded-full border',
                  'transition-[border-color,box-shadow] duration-150 active:scale-95',
                  'p-2 transition-all hover:border-rose-500/25',
                  'hover:bg-rose-500/10 hover:text-rose-500 focus:ring-2',
                  'focus:ring-offset-2 focus:outline-none disabled:pointer-events-none',
                  'hover:drop-shadow-xl hover:drop-shadow-rose-500/25',
                )}
...
const ModalHeader = ({
  ...
  <div
    className={cn(
      'bg-background dark:bg-background/98 relative flex flex-col space-y-1.5 p-6 text-center sm:rounded-t-3xl sm:text-left',
      className,
    )}
    {...props}
  >
    {children}
    {actions && (
      <div className="absolute top-4.5 right-[64px] flex h-full max-h-[33px] min-h-[33px] items-center gap-3">
        {actions}
      </div>
    )}
  </div>
...
const ModalFooter = ({
  ...
  <div
    className={cn(
      'bg-background dark:bg-background/98 flex flex-col-reverse p-6 sm:flex-row sm:justify-end sm:space-x-2',
      className,
    )}
    {...props}
  />
...
const ModalBody = ({
  ...
  <div
    className={cn('bg-background/95 flex flex-1 flex-col p-6 py-6', className)}
    {...props}
  />
);
